# -*- coding:utf-8 -*-
import unittest
from studio.lp_dataocean_handle.lp_application_on_plus import Handle
...
class TestPlugin(unittest.TestCase):
    ...
    def test_application_on_true(self):
        ...
        handler = Handle(data)
        res = handler.handle()
        self.assertEquals(res.get('application_on_plus'), "12:20")

    def test_application_on_default(self):
        data = {}
        handler = Handle(data)
        res = handler.handle()
        self.assertEquals(res.get('application_on_plus'), StringTypeDefault)