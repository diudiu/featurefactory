...
import unittest
import json
from studio.lp_dataocean_handle.lp_cur_corp_years import Handle
...
start_business_dates = ["2009-09-08", "2015-09-08", "2005-09-08", "2009.9.8", "", "None"]
...
class TestPlugin(unittest.TestCase):
    ...
    def test_lp_cur_corp_years(self):
        data = self.data.copy()
        a = open('res.txt', 'w+')
        for start_business_date in start_business_dates:
            data['content']['start_business_date'] = start_business_date
            handler = Handle(data)
            res = handler.handle()
